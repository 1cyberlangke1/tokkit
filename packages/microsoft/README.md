...
Microsoft Phi family 的 tokkit 子包，只包含当前 MIT 兼容、且能用官方 `tokenizer.json` 直接验证的纯文本 tokenizer。

## 支持的 family

- `nextcoder`：覆盖 `NextCoder-7B / 14B / 32B`
- `phi-1`：覆盖 `phi-1 / phi-1_5 / phi-2`
- `phi-3-mini`：覆盖 `Phi-3-mini-4k-instruct / Phi-3-mini-128k-instruct`，以及共享同一 tokenizer 的 `MediPhi / MediPhi-Instruct`
...
这两条官方文本主线仓库当前没有直接公开根目录 `tokenizer.json`，不满足当前“下载官方 tokenizer 快照并直接对拍”的接入门槛。
...
const tokenizer = await getTokenizer("phi-3.5")
